...
for M, N in test_configs:
    ...
    model_o = nn_parity(B, N,  hidden=M*M, num_epochs=M*100,lr=1e-3,
                        excite=list(range(1,N+1,2)))
    model_e = nn_parity(B, N,  hidden=M*M, num_epochs=M*100,lr=1e-3,
    ...
    for _ in range(num_tests):
        ...
        test_e_tensor = torch.tensor(test_e, dtype=torch.float32).unsqueeze(0)
        recovered_nn = (
            model_o.decode(test_e_tensor)
            if sum(test_e) % 2
            else model_e.decode(test_e_tensor)
        )
        # recovered_nn = model.decode(test_e_tensor)
        recovered_nn = torch.round(recovered_nn).detach().numpy().astype(np.int32)
...
with open('decoding_results_NN.txt', 'w') as f:
...
try:
    previous_results = parse_result_file('decoding_results_4NN.txt')
...
ax1.set_title('Average Decoding Times')
...
ax2.set_title('Decoding Success Rates')
ax2.legend()
ax2.grid(True)
