...
def my_molecule(symbols, coordinates, basis, mult):
    ...
    for i, symbol in enumerate(symbols): 
        atoms.append(f"{symbol} {coordinates[i, 0]} {coordinates[i, 1]} {coordinates[i, 2]}")
    mol = pyscf.M(atom = atoms, 
                  basis = basis, 
                  spin = (mult-1)/2,
                  symmetry = True)
    return mol
