import numpy as np
import qiskit
from lib.diag import *
from lib.linear import *
from lib.pre_FED import *
import torch
...
def generate_binary_vectors(M,Q,N,excite,sample,  bias=[]):
    '''
    Generate Training data
    '''
    binary_vectors=set()
    for i in bias:
        binary_vectors.add(tuple(i))
...
def loss_function(recon_x, x, Q):
    BCE=nn.functional.binary_cross_entropy(recon_x, x.view(-1, Q), reduction='sum')
    return BCE

def nn_parity(B, N, depth=1, hidden=0, model=None, num_epochs=1000, lr=1e-2, excite=[], sample=[], bias=[]):
    '''
    ...
    for epoch in range(num_epochs//2):
        ...
        dataset = generate_binary_vectors(M,Q,N,excite,sample,bias)
    ...
    for epoch in range(num_epochs//2):
        ...
        # if epoch%100==0:
        #     print(f'Epoch {epoch+1}/{num_epochs}, Loss: {loss}')
    print(f'Epoch {epoch+num_epochs//2+1}/{num_epochs}, Loss: {loss}')
    
    return model.eval()

def nn_str_decode(string, model_set, B):
    '''
    A function to decode string: 

    Input:
    ------
    code: list of string to decode

    table: the dictionary generated

    Output:
    decoded: list of decoded word
    '''
    count=1
    for i in model_set.keys():
        ham=to10(string)
        hw=sum(ham)
        if hw == i:
            return string+'0'*(model_set[i].M-model_set[i].Q)
        model=model_set[i]
        edata=[]
        edata.append(ham)
        edata=torch.tensor(edata,dtype=torch.float32).to(device)
        recon_batch = model.decode(edata)
    # Convert the output probabilities to binary vectors
        binary_vectors = torch.round(recon_batch).detach().cpu()
        s1 = ''.join(str(int(x.numpy())) for x in binary_vectors[0])
        s2 = B.dot(binary_vectors[0])%2
        if sum(abs(s2-ham))==0 and sum(to10(s1))==model.N:
            decoded=s1
            count=0
            return decoded
    if count:
...
def nn_str_decode_general(string, model_set, B):
    '''
    A function to decode string: 
    ...
    # Convert the output probabilities to binary vectors
        ...
        s1 = ''.join(str(int(x.numpy())) for x in binary_vectors[0])
        s2 = B.dot(binary_vectors[0])%2
        if sum(abs(s2-to10(string)))==0 and sum(to10(s1))==model.N:
            decoded=s1
            count=0
            return decoded
    if count:
        return model.dummy
def nn_decoder(string, G, ustring, op, model_set, hardcore):
    '''
    Decode only the ordered rdms.
    ...
    d = ''.join(np.array(d, dtype = str))
    d2 = ''.join(np.array(d2, dtype = str))
    d = nn_str_decode(d, model_set, G)
    d2 = nn_str_decode(d2, model_set, G)
    d =  np.array(list(d), dtype = int)
    d2  = np.array(list(d2), dtype = int)
    'convert back to electronic pairs'
    if hardcore:
...
def nn_hist_decoder(hist, G, ustring, op, model, hardcore):
    '''
    Input:
    ------
    hist: the sampled result.
    ...
    signs = []
    for i in hist.keys():
        signs.append(nn_decoder(i, G, ustring, op, model, hardcore))

    return np.array(signs)@np.array(list(hist.values()))

def nn_rdm_val(experiments, model, groups, signs, G, hardcore):
    '''
    test my decoder.
    ...
    for i in range(len(groups)):
        ...
        for j in range(len(groups[i])):
            ...
            loc = groups[i][j]
            sign = signs[i][j]
            val = nn_hist_decoder(experiment, G, ustring, loc, model, hardcore)
            if sum(ustring)!=0:
                val/=2
            val = sign*val
            vals.append(val)
...
def process_group(group_args):
    experiment, model, group, signs, G, hardcore = group_args
    vals = []
    ustring =xfun(group[0], G.shape[1]*2**hardcore)
    if hardcore:
        ustring = np.reshape(ustring, (ustring.shape[0]//2, 2))
        ustring = ustring.T[0]
    da = np.zeros((G.shape[1], G.shape[1]))
...
def nn_rdm_val_parallel(experiments, model, groups, signs, G, hardcore):
    num_processes = 4  # Adjust as needed

    with Pool(num_processes) as pool:
        group_args_list = [(experiments[i], model, group, signs[i], G, hardcore) for i, group in enumerate(groups)]
        results = pool.map(process_group, group_args_list)

    combined_da = np.zeros((G.shape[1], G.shape[1]))
    combined_ddaa = np.zeros((G.shape[1], G.shape[1], G.shape[1], G.shape[1]))
    combined_Vals = []

    for da, ddaa, vals in results:
        combined_da += da
        combined_ddaa += ddaa
        combined_Vals.extend(vals)
    pool.close()
    return combined_da, combined_ddaa, combined_Vals
