...
def state_to_histogram(state, basis, G, ustring):
    ...
    if sum(ustring) != 0:
        ...
        for i in basis:
            ibasis = (G@i)%2
            istr = ''.join(np.array(np.array(ibasis, dtype = int), dtype = str))
            index=to10([istr])[0]
            cbasis = (cnot@G@i)%2
...
def nn_parity(B, N, depth=1, model=None, num_epochs=1000, lr=1e-2, excite=[], sample=[]):
    '''
    ...
    for epoch in range(num_epochs//2):
        ...
        losses.append(epoch_loss)
        if epoch%10==0:
            print(f'Epoch {epoch+num_epochs//2+1}/{num_epochs}, Loss: {epoch_loss}')
    
    return model.eval()

def nn_str_decode(string, model):
    '''
    A function to decode string: 
    ...
    '''
    if sum(to10(string))==model.N:
        return string+'0'*(model.M-model.Q)
    ...
    binary_vectors = torch.round(recon_batch).detach()
    s1=''.join(str(int(x.numpy())) for x in binary_vectors[0])
    if sum(np.array(list(s1))=='1') != model.N:
        decoded=model.dummy
    else:
        decoded=s1
    return decoded

def nn_decoder(string, G, ustring, op, model, hardcore):
    '''
    Decode only the ordered rdms.
    ...
    d = ''.join(np.array(d, dtype = str))
    d2 = ''.join(np.array(d2, dtype = str))
    d = nn_str_decode(d, model)
    d2 = nn_str_decode(d2, model)
    d =  np.array(list(d), dtype = int)
    d2  = np.array(list(d2), dtype = int)
